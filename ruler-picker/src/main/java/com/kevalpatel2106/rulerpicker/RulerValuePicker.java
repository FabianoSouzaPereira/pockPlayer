...
public final class RulerValuePicker extends FrameLayout implements ObservableHorizontalScrollView.ScrollChangedListener, View.OnTouchListener, ObservableHorizontalScrollView.ScrollStartListener, RulerView.onFinishLoadTimeline {

    private ScaleGestureDetector mScaleDetector;
    private GestureDetector mGestureDetector;
    private float mScaleFactor = 1F;
    private float mIndicatorInterval = 1F;
    private Long mPosition = (long) -1;
    /**
    ...
    private void init(@Nullable AttributeSet attributeSet) {
        ...
        mIndicatorInterval = mRulerView.getIndicatorIntervalWidth();
    ...
    private void addChildViews() {
        mHorizontalScrollView = new ObservableHorizontalScrollView(getContext(), this, this);
        ...
        mRulerView = new RulerView(getContext(), this);
        ...
        mScaleDetector = new ScaleGestureDetector(getContext(), new ScaleGestureDetector.SimpleOnScaleGestureListener()
        {
            ...
            public boolean onScale(ScaleGestureDetector detector)
            {
                ...
                if (mScaleFactor <  1F) // Minimum scale condition:
                    mScaleFactor =  1F;

                if (mScaleFactor > 15F) // Maximum scale condition:
                    mScaleFactor = 15F;

                int size = 0;

                if ((int)(mIndicatorInterval * mScaleFactor) != mRulerView.getIndicatorIntervalWidth()){
                    //rulerContainer.setLayoutParams(new LinearLayout.LayoutParams((int) (rulerContainer.getWidth() * mIndicatorInterval), rulerContainer.getHeight()));
                    //removeAllViews();
                    //Log.i("TE mHorizontal://", "width: "+mHorizontalScrollView.getWidth()+"Measured width: "+mHorizontalScrollView.getMeasuredWidth());
                    //Log.i("TE mRuler://", "width: "+mRulerView.getWidth()+"Measured width: "+mRulerView.getMeasuredWidth());
                    //rulerContainer.addView(mRulerView);
                } else {
                    //size = mRulerView.getWidth();
                }

                mRulerView.mShowScaleMinutes = mScaleFactor;
                //mHorizontalScrollView.measure((int) (mHorizontalScrollView.getMeasuredWidth() * mRulerView.getIndicatorIntervalWidth()), mHorizontalScrollView.getMeasuredHeight());
                //mRulerView.setLayoutParams(new LinearLayout.LayoutParams(size , LayoutParams.WRAP_CONTENT));
                //mRulerView.requestLayout();
                //mHorizontalScrollView.requestLayout();
                //mHorizontalScrollView.invalidate();
                //invalidate();
                //requestLayout();

                int posX = getCurrentValue();

                mRulerView.getWidth();
                mRulerView.setIndicatorIntervalDistance((int) (mIndicatorInterval * mScaleFactor));
                mRulerView.requestLayout();

                //mHorizontalScrollView.setScrollX(mHorizontalScrollView.getScrollX() * mRulerView.getIndicatorIntervalWidth());

                mHorizontalScrollView.scrollTo(posX * mRulerView.getIndicatorIntervalWidth(), 0);
                return true;
            }
        });
        rulerContainer.setOnTouchListener(RulerValuePicker.this);
        mHorizontalScrollView.scrollTo(getPositionNow() * mRulerView.getIndicatorIntervalWidth(), 0);
    }

    private int getPositionNow() {
        Calendar dateNow = Calendar.getInstance();
        Calendar dateStartToday = Calendar.getInstance();
        dateStartToday.set(dateNow.get(Calendar.YEAR),
                dateNow.get(Calendar.MONTH),
                dateNow.get(Calendar.DAY_OF_MONTH), 0,0,0);



        int dayMinutes = (int) ((dateNow.getTimeInMillis() /1000) -
                (dateStartToday.getTimeInMillis() / 1000));
    ...
    private void calculateNotchPath() {
        ...
        mNotchPath.moveTo(getWidth() / 2, 0);
    ...
    public void onScrollStopped() {
        if (mRulerView.mTimelineData.size() > 0 && getCurrentValue() < mRulerView.mTimelineData.get(0).getStart()){
            mHorizontalScrollView.smoothScrollTo((int) (mRulerView.mTimelineData.get(0).getStart() * mRulerView.getIndicatorIntervalWidth()), 0);
        } else {
            boolean forcePosition = true;

            if (mRulerView.mTimelineData.size() > 0 &&
                    getCurrentValue() > mRulerView.mTimelineData.get(0).start &&
                    getCurrentValue() < mRulerView.mTimelineData.get(mRulerView.mTimelineData.size() -1).start +
                            mRulerView.mTimelineData.get(mRulerView.mTimelineData.size() -1).duration) {
                for (int i = 0; i < mRulerView.mTimelineData.size(); i++) {
                    if (getCurrentValue() >= mRulerView.mTimelineData.get(i).start &&
                            getCurrentValue() < (mRulerView.mTimelineData.get(i).start + mRulerView.mTimelineData.get(i).duration)) {
                        break;
                    } else if (mRulerView.mTimelineData.size() > (i) &&
                            getCurrentValue() < mRulerView.mTimelineData.get(i).start &&
                            (getCurrentValue() < mRulerView.mTimelineData.get(i).start +
                                    mRulerView.mTimelineData.get(i).duration)) {
                        mHorizontalScrollView.smoothScrollTo((int) (mRulerView.mTimelineData.get(i).getStart() * mRulerView.getIndicatorIntervalWidth()), 0);
                        forcePosition = false;
                        break;
                    }
                }
    ...
    public Parcelable onSaveInstanceState() {
        ...
        if(mRulerView.getMeasuredWidth() == 0){
            ss.value = getPositionNow();
        } else {
    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        ...
        return (mScaleDetector.onTouchEvent(motionEvent) || super.onTouchEvent(motionEvent));
    }

    @Override
    public void onFinishedDrawScroll() {
        Long position = (mPosition != -1) ? mPosition : getPositionNow();
        mHorizontalScrollView.smoothScrollTo((int) (position * mRulerView.getIndicatorIntervalWidth()),0);
        mPosition = (long) -1;
    }

    public void scrollToPosition(Long position) {
        mPosition = position;
        mHorizontalScrollView.smoothScrollTo((int) (mPosition * mRulerView.getIndicatorIntervalWidth()),0);
    }

    @Override
    public void onFinishedLoadTimeline() {
        if(getCurrentValue() < mRulerView.mTimelineData.get(0).getStart()){
            mHorizontalScrollView.smoothScrollTo((int) (mRulerView.mTimelineData.get(0).getStart() * mRulerView.getIndicatorIntervalWidth()),0);
        }
    }
    ...
    public void setDataTimeline(HashMap<Long, Long> hash) {
        mRulerView.setDataTimeline(hash);
    }
